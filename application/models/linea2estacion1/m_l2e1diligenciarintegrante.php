...
class M_l2e1diligenciarintegrante extends CI_Model {
    ...
    {        
        $query = $this->db->query('call splistartabladiligenciarintegrantel2e1('.$mfolio.');');//se llama por procedimiento l almacenado
    ...
    public function fm_estadofin($mfolio)
    { 
        $query = $this->db->query('call spactivarbotoncerrarform('.$mfolio.');');
        $resultado = $query->result();
        $query->next_result(); //NO SE TOCAN Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        $query->free_result(); //NO SE TOCAN  Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        return $resultado;        
    ...
    {
        $query = $this->db->query('call spbuscarppcapitulo1p1n5('.$mfolio.');');
        $resultado = $query->result();
        $query->next_result(); //NO SE TOCAN Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        $query->free_result(); //NO SE TOCAN  Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        return $resultado;        
    ...
    public function fm_llenarcapitulosintegrantel2e1($mfolio,$midintegrante)
    {
        $this->db->query('call sp2llenarcapitulosintegrantel2e1('.$mfolio.','.$midintegrante.');');
    }
