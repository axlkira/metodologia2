...
class M_diligenciarintegranteintermedia extends CI_Model {
    ...
    {        
        $query = $this->db->query('call splistartabladiligenciarintegranteintermedia('.$mfolio.');');//se llama por procedimiento l almacenado
    ...
    { 
        $query = $this->db->query('call spactivarbotoncerrarformintermedia('.$mfolio.');');
        $resultado = $query->result();
        $query->next_result(); //NO SE TOCAN Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        $query->free_result(); //NO SE TOCAN  Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        return $resultado;        
    ...
    public function fm_insertarcapintegrantesintermedia($mfolio,$midintegrante) {
            $this->db->query('call spinsertacapitulointegranteintermedia('.$mfolio.','.$midintegrante.');');
    }
