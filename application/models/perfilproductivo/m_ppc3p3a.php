...
class M_ppc3p3a extends CI_Model {
   ...
   public function fm_cargardatosgenerales($mfolio,$midintegrante)
    ...
    {
        $query = $this->db->query('call spbuscarppcapitulo3p3a('.$midintegrante.');');
        $resultado = $query->result();
        $query->next_result(); //NO SE TOCAN Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        $query->free_result(); //NO SE TOCAN  Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        return $resultado;        
    ...
    public function fm_actualizarpregunta($mfolio,$midintegrante,$cvvalorppc3p3a)
    {
        $this->db->query('call spactualizarppcapitulo3p3a('.$mfolio.','.$midintegrante.',"'.$cvvalorppc3p3a.'");');
    }    
