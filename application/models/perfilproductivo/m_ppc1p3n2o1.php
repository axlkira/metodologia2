...
class M_ppc1p3n2o1 extends CI_Model {
    ...
    {
        $query = $this->db->query('call sp3ayudas("ppc1p3n4o1");');//se coloca el codigo de la ayuda
    ...
    {
        $query = $this->db->query('call spbuscarppcapitulo1p3n2o1('.$midintegrante.');');
        $resultado = $query->result();
        $query->next_result(); //NO SE TOCAN Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        $query->free_result(); //NO SE TOCAN  Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        return $resultado;        
    ...
    {
        $this->db->query('call spactualizarppcapitulo1p3n2o1('.$mfolio.','.$midintegrante.',"'.$mvalorppc1p3n2.'","'.$mppc1p3n5institucion.'","'.$mppc1p3n5area.'","'.$mppc1p3n6.'","'.$mppc1p3n7.'");');
    }    
    ...
    {
        $this->db->query('call speliminarppcapitulo1p3n2o1('.$mfolio.','.$midintegrante.');');
    }   
       
    public function fm_insertarotro1($mfolio,$midintegrante)
    {
        $this->db->query('call spinsertarotro1ppcapitulo1p3n2('.$mfolio.','.$midintegrante.');');
    }  
}      