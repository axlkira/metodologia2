...
class M_actualizarpp extends CI_Model {
    ...
    {        
        $query = $this->db->query('call splistarrepresentante('.$folio.');');//se llama por procedimiento l almacenado
    ...
    public function fm_listarmotivo()
    {        
        $query = $this->db->query('call spmotivo();');//se llama por procedimiento l almacenado
        $resultado = $query->result();
        $query->next_result(); // Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        $query->free_result(); // Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        return $resultado;
    ...
    public function fm_actualizoexiste($folio)
    {
        $query = $this->db->query('call spactualizoexiste ('.$folio.');');
        $resultadoactualizo = $query->result();
        $query->next_result(); //NO SE TOCAN Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        $query->free_result(); //NO SE TOCAN  Estas dos funciones permiten realizar varias consultas en el mismo controlador.
        return $resultadoactualizo;        
    }
    ...
    public function fm_actualizarh($mfolio,$mestrato,$mcomuna,$mbarrio,$mdireccion,$mtelefono,$mcelular,$mtelfcont,$mcelcont)
    {
        $this->db->query('call spactualizarhogarpp ('.$mfolio.',"'.$mestrato.'","'.$mcomuna.'","'.$mbarrio.'","'.$mdireccion.'","'.$mtelefono.'","'.$mcelular.'","'.$mtelfcont.'","'.$mcelcont.'","'.date("Y-m-d H:i:s").'");');
    }    
