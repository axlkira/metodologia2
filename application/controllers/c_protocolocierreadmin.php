...
class  C_protocolocierreadmin extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->helper('url');
        $this->load->library('session');
        $this->load->model('m_protocolocierreadmin');//carga el controlador modelo
    ...
    public function fc_protocolocierreadmin($page = 'v_protocolocierreadmin') {
        ...
        $datosintegrante = $this->m_protocolocierreadmin->fm_datosintegrante($folio,$idintegrante);//lo mandasmos a la funcion del modelo para verificar
      ...
      $idvisita = '91';
      $datosidvisita = $this->m_protocolocierreadmin->fm_traerp0informaciongeneral($folio,$idvisita);//lo mandasmos a la funcion del modelo para verificar
      ...
      //INICIO TRAER DATOS DE LA VISITA ENCUADRE
      $idvisitaAccion = '91';
      $datosidvisitaAccion = $this->m_protocolocierreadmin->fm_traerp0informaciongeneralAccion($folio,$idvisitaAccion);//lo mandasmos a la funcion del modelo para verificar
      ...
      ///////////////////// FIN NUEVO ACCIONES/////////////////
       // TRAE EL ULTIMO PASO REALIZADO DE LA ESTACIÓN
          $maximoidpaso = $this->m_protocolocierreadmin->mostrarmaximoidpaso($folio);
       ...
       //ESTO LEVANTA LA VISTA     
        $titulo = 'v_protocolocierreadmin'; // para el titulo de la vista en la pestaña
        $this->load->view('pages/' . $page,  array('titulo' => $titulo, 'foot' => FOOTS, 'head3' => HEAD3, 'botonerag' => BOTONERAG, 'botoneraa' => BOTONERAA, 'folio' => $folio,'tdatinte' => $tdatinte, 'midpaso' => $midpaso, 'doccogestor' => $doccogestor, 'tdatidvisita' => $tdatidvisita, 'tdatidvisitaAccion' => $tdatidvisitaAccion));
    ...
    public function fc_cerrarpaso()
    {
       ...
       $this->m_protocolocierreadmin->fm_cerrarpaso($cfolio,$cidpaso,$cestado);
    }
    ...
    public function fc_cerrarestacion()
    {
       ...
       $this->m_protocolocierreadmin->fm_cerrarestacion($cfolio,$cidestacion,$cestado,$this->session->userdata('id'),$this->session->userdata('documento'),$clongitud,$clatitud,$cmensajedonde);
    }
    ...
    public function fc_insertarfolioexitoso()
    {
       $cidestacion = $this->input->get('videstacion');       
       $cfolio = $this->input->get('vfolio');       
       $this->m_protocolocierreadmin->fm_insertarfolioexitoso($cfolio,$cidestacion);
    }
    ////////////////// nuevo /////////////////
  public function fc_actualizarep0()
  {
     ...
     $fechaIni = $this->m_protocolocierreadmin->fm_actualizarep0($cfolio,$cidestacion,$cvalorp0,$cvcualmotivo,$cdoccogestor);
...
public function fc_actualizarep0Accion()
{
   ...
   $cdoccogestor = $this->input->get('vdoccogestor');
   $this->m_protocolocierreadmin->fm_actualizarep0Accion($cfolio,$cidestacion,$cvalorp0Accion,$cvcualmotivoAccion,$cdoccogestor);
}
