...
class  C_protocolol3sc extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->helper('url');
        $this->load->library('session');
        $this->load->model('m_protocolol3sc');//carga el controlador modelo
    ...
    public function fc_protocolol3sc($page = 'v_protocolol3sc') {
        ...
        $datosintegrante = $this->m_protocolol3sc->fm_datosintegrante($folio,$idintegrante);//lo mandasmos a la funcion del modelo para verificar
      ...
      $idvisita = '31';
      $datosidvisita = $this->m_protocolol3sc->fm_traerp0informaciongeneral($folio,$idvisita);//lo mandasmos a la funcion del modelo para verificar
      ...
      //INICIO TRAER DATOS DE LA VISITA ENCUADRE
      $idvisitaAccion = '31';
      $datosidvisitaAccion = $this->m_protocolol3sc->fm_traerp0informaciongeneralAccion($folio,$idvisitaAccion);//lo mandasmos a la funcion del modelo para verificar
      ...
      ///////////////////// FIN NUEVO ACCIONES/////////////////
       ...
       // TRAE EL ULTIMO PASO REALIZADO DE LA ESTACIÓN
          $maximoidpaso = $this->m_protocolol3sc->mostrarmaximoidpaso($folio);
        ...
        // INICIO TRAE LA ULTIMA ESTACIÓN
        $idestacion31 = $this->m_protocolol3sc->mostrarestacion31($folio);
        $midestacion31= array('folio' => '');
        
        foreach ($idestacion31 as $filainte)
          {
          $midestacion31 = array('folio' => $filainte->folio); 
          }
        
      //FIN TRAE LA ULTIMA ESTACIÓN
        
     
       //ESTO LEVANTA LA VISTA     
        $titulo = 'v_protocolol3sc'; // para el titulo de la vista en la pestaña
        $this->load->view('pages/' . $page,  array('titulo' => $titulo, 'foot' => FOOTS, 'head3' => HEAD3, 'botonerag' => BOTONERAG, 'botoneraa' => BOTONERAA, 'folio' => $folio,'tdatinte' => $tdatinte, 'midpaso' => $midpaso, 'doccogestor' => $doccogestor, 'tdatidvisita' => $tdatidvisita, 'tdatidvisitaAccion' => $tdatidvisitaAccion, 'midestacion31' => $midestacion31));
    ...
    public function fc_cerrarpaso()
    {
       ...
       $this->m_protocolol3sc->fm_cerrarpaso($cfolio,$cidpaso,$cestado);
    }
    ...
    public function fc_cerrarestacion()
    {
       ...
       $this->m_protocolol3sc->fm_cerrarestacion($cfolio,$cidestacion,$cestado,$this->session->userdata('id'),$this->session->userdata('documento'),$clongitud,$clatitud,$cmensajedonde);
    }
    ...
    public function fc_insertarfolioexitoso()
    {
       $cidestacion = $this->input->get('videstacion');       
       $cfolio = $this->input->get('vfolio');       
       $this->m_protocolol3sc->fm_insertarfolioexitoso($cfolio,$cidestacion);
    }
  ////////////////// nuevo /////////////////
  public function fc_actualizarep0()
  {
     ...
     $fechaIni = $this->m_protocolol3sc->fm_actualizarep0($cfolio,$cidestacion,$cvalorp0,$cvcualmotivo,$cdoccogestor);
...
public function fc_actualizarep0Accion()
{
   ...
   $cdoccogestor = $this->input->get('vdoccogestor');
   $this->m_protocolol3sc->fm_actualizarep0Accion($cfolio,$cidestacion,$cvalorp0Accion,$cvcualmotivoAccion,$cdoccogestor);
}
