...
class  C_c1p1ap12 extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->helper('url');
        $this->load->library('session');
        $this->load->model('m_c1p1ap12');//carga el controlador modelo
        //$this->load->model('m_c9mp1ap5');//carga el controlador modelo
    }

    //funcion encargada de llamar a la vista de la pagina 
    public function fc_c1p1ap12($page = 'v_c1p1ap12') {
        ...
        $datosintegrante = $this->m_c1p1ap12->fm_datosintegrante($folio,$idintegrante);//lo mandasmos a la funcion del modelo para verificar
        ...
        $folioexiste = $this->m_c1p1ap12->fm_folioexiste($folio);//lo mandasmos a la funcion del modelo para verificar
        ...
        //ACA USTEDES SI TOCAN - COLOCAN EL NOMBRE DE LAS VARIABLES idresc9mp1 - idresc9mp4a - idresc9mp4b - idresc9mp4c
           $foexiste= array('efolio' => '', 'ec1p1' =>'', 'ec1p2' =>'', 'ec1p3' =>'', 'eestadoc1p3' => '', 'ec1p4' => '', 'eestadoc1p4' => '', 'ec1p5' => '', 'eestadoc1p5' => '',
                            'ec1p6' => '', 'ec1p7' => '','ec1p8' => '','ec1p9' => '','ec1p10' => '', 'eestadoc1p10' => '','ec1p11' => '','ec1p11a' => '','ec1p12' => '','ec1p13' => ''); 
        ...
        $foexiste = array('efolio' => $fila->folio, 'ec1p1' => $fila->c1p1, 'ec1p2' => $fila->c1p2, 'ec1p3' => $fila->c1p3, 'eestadoc1p3' => $fila->estadoc1p3, 'ec1p4' => $fila->c1p4, 'eestadoc1p4' => $fila->estadoc1p4,
                          ...
                          'ec1p9' => $fila->c1p9, 'ec1p10' => $fila->c1p10 , 'eestadoc1p10' => $fila->estadoc1p10, 'ec1p11' => $fila->c1p11, 'ec1p11a' => $fila->c1p11a,'ec1p12' => $fila->c1p12,'ec1p13' => $fila->c1p13); //se guarda el resultado del ption value en la variable datos
        ...
        $listasino = $this->m_c1p1ap12->fm_listarestrato();
        $estratos = '';
        ...
        //traer una lista de valores
        $listasino = $this->m_c1p1ap12->fm_listarrelcont();
        $relcont = '';
        foreach ($listasino as $estado)
        {
        ...
        //traer una lista de valores
        $listacomunas = $this->m_c1p1ap12->fm_listarcomunas();
        $comunas = '';
        ...
        //traer una lista de valores
//        $listasino = $this->m_c1p1ap12->fm_listarbarrios();
        $barrios = '';
//        foreach ($listasino as $estado)
//        {
...
//        }
       ...
       //ESTO LEVANTA LA VISTA     
        $titulo = 'v_c1p1ap12'; // para el titulo de la vista en la pestaña
        $this->load->view('pages/' . $page,  array('titulo' => $titulo, 'foot' => FOOTS, 'head1' => HEAD1, 'botonerag' => BOTONERAG, 'botoneraa' => BOTONERAA,  'estratos' => $estratos , 'relcont' => $relcont,'comunas' => $comunas,'barrios' => $barrios, 'folio' => $folio, 'foexiste' =>$foexiste, 'tdatinte' => $tdatinte));
    ...
    public function fc_traerbarrios()
    {
        ...
        $listasino = $this->m_c1p1ap12->fm_listarbarrios($cidcomuna);
        $barrios = '';
    ...
    public function fc_insertar()
    {
        ...
        $c1p13 = $this->input->get('vc1p13');        
        $this->m_c1p1ap12->fm_insertar($cfolio,$c1p1,$c1p2,$c1p3,$c1p4,$c1p5,$c1p6,$c1p7,$c1p8,$c1p9,$c1p10,$c1p11,$c1p11a,$c1p12,$c1p13);
    ...
    public function fc_actualizar()
    {
        ...
        $c1p12 = $this->input->get('vc1p12');
        $c1p13 = $this->input->get('vc1p13');        
        $this->m_c1p1ap12->fm_actualizar($cfolio,$c1p1,$c1p2,$c1p3,$c1p4,$c1p5,$c1p6,$c1p7,$c1p8,$c1p9,$c1p10,$c1p11,$c1p11a,$c1p12,$c1p13);
        //echo json_encode(array('result' => 'guardado'));
     ...
     public function fc_cerrarcapitulo()
   {
       ...
       $this->m_c1p1ap12->fm_cerrar($cfolio,$ccerrarcap1);
   }
   
   
     public function fc_unipersonal()
   {
       $cfolio = $this->input->get('vfolio');       
       $this->m_c1p1ap12->fm_unipersonal($cfolio);
   }
   
     public function fc_unipersonalborrar()
   {
       $cfolio = $this->input->get('vfolio');       
       $this->m_c1p1ap12->fm_unipersonalborrar($cfolio);
   }
