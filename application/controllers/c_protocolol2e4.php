...
class  C_protocolol2e4 extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->helper('url');
        $this->load->library('session');
        $this->load->model('m_protocolol2e4');//carga el controlador modelo
    ...
    public function fc_protocolol2e4($page = 'v_protocolol2e4') {
        ...
        $datosintegrante = $this->m_protocolol2e4->fm_datosintegrante($folio,$idintegrante);//lo mandasmos a la funcion del modelo para verificar
      ...
      $idvisita = '24';
      $datosidvisita = $this->m_protocolol2e4->fm_traerp0informaciongeneral($folio,$idvisita);//lo mandasmos a la funcion del modelo para verificar
      ...
      //INICIO TRAER DATOS DE LA VISITA ENCUADRE
      $idvisitaAccion = '24';
      $datosidvisitaAccion = $this->m_protocolol2e4->fm_traerp0informaciongeneralAccion($folio,$idvisitaAccion);//lo mandasmos a la funcion del modelo para verificar
      ...
      ///////////////////// FIN NUEVO ACCIONES/////////////////
       // TRAE EL ULTIMO PASO REALIZADO DE LA ESTACIÓN
          $maximoidpaso = $this->m_protocolol2e4->mostrarmaximoidpaso($folio);
     ...
     $cdatositi = $this->m_protocolol2e4->fm_traervaloriti();
...
//fin traer valores porcentaje de logros
       ...
       //ESTO LEVANTA LA VISTA     
        $titulo = 'v_protocolol2e4'; // para el titulo de la vista en la pestaña
    ...
    public function fc_cerrarpaso()
    {
       ...
       $this->m_protocolol2e4->fm_cerrarpaso($cfolio,$cidpaso,$cestado);
    }
    ...
    public function fc_cerrarestacion()
    {
       ...
       $this->m_protocolol2e4->fm_cerrarestacion($cfolio,$cidestacion,$cestado,$this->session->userdata('id'),$this->session->userdata('documento'),$clongitud,$clatitud,$cmensajedonde);
    }
    ...
    public function fc_insertarfolioexitoso()
    {
       $cidestacion = $this->input->get('videstacion');       
       $cfolio = $this->input->get('vfolio');       
       $this->m_protocolol2e4->fm_insertarfolioexitoso($cfolio,$cidestacion);
    }
  ...
  public function fc_actualizarep0()
  {
     ...
     $fechaIni = $this->m_protocolol2e4->fm_actualizarep0($cfolio,$cidestacion,$cvalorp0,$cvcualmotivo,$cdoccogestor);
...
public function fc_actualizarep0Accion()
{
   ...
   $cdoccogestor = $this->input->get('vdoccogestor');
   $this->m_protocolol2e4->fm_actualizarep0Accion($cfolio,$cidestacion,$cvalorp0Accion,$cvcualmotivoAccion,$cdoccogestor);
}
