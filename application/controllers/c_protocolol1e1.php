...
class  C_protocolol1e1 extends CI_Controller {
    ...
    public function fc_protocolol1e1($page = 'v_protocolol1e1') {
        ...
        $datosintegrante = $this->m_protocolol1e1->fm_datosintegrante($folio,$idintegrante);//lo mandasmos a la funcion del modelo para verificar
      ...
      $idvisita = '11';
      $datosidvisita = $this->m_protocolol1e1->fm_traerp0informaciongeneral($folio,$idvisita);//lo mandasmos a la funcion del modelo para verificar
      ...
      // TRAE EL ULTIMO PASO REALIZADO DE LA ESTACIÓN
          $maximoidpaso = $this->m_protocolol1e1->mostrarmaximoidpaso($folio);
       ...
       //ESTO LEVANTA LA VISTA     
        $titulo = 'v_protocolol1e1'; // para el titulo de la vista en la pestaña
        $this->load->view('pages/' . $page,  array('titulo' => $titulo, 'foot' => FOOTS, 'head' => HEAD1, 'botonerag' => BOTONERAG, 'botoneraa' => BOTONERAA, 'folio' => $folio,'tdatinte' => $tdatinte, 'midpaso' => $midpaso, 'doccogestor' => $doccogestor, 'tdatidvisita' => $tdatidvisita));
    ...
    public function fc_calcularlogros()
    {
       $cfolio = $this->input->get('vfolio');       
       $this->m_protocolol1e1->fm_calcularlogros($cfolio);
    }
    
    
    public function fc_calcularprivaciones()
    {
       $cfolio = $this->input->get('vfolio');       
       $this->m_protocolol1e1->fm_calcularprivaciones($cfolio);
    }
  ...
  public function fc_actualizarep0()
  {
     ...
     $fechaIni = $this->m_protocolol1e1->fm_actualizarep0($cfolio,$cidestacion,$cvalorp0,$cvcualmotivo,$cdoccogestor);
