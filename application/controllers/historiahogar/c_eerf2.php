...
class  C_eerf2 extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->helper('url');
        $this->load->library('session');
        $this->load->model('historiahogar/m_eerf2');//carga el controlador modelo
        $this->load->model('m_protocolol1e1');//carga el controlador modelo
    }

    //funcion encargada de llamar a la vista de la pagina 
    public function fc_eerf2($page = 'v_eerf2') {
        ...
        $cdatosgenerales = $this->m_eerf2->fm_cargardatosgenerales($cfolio);
        ...
        $cdatosintegrante = $this->m_eerf2->fm_cargarintegtranteshogar($cfolio);
        ...
        $eerf = 0;
        $crespuestametas = $this->m_eerf2->fm_buscarmetas($cfolio);
        foreach ($crespuestametas as $cresmetas)
        {
            //$carraymetas = array('efolio'=>$cresmetas->folio,'eeerf' =>$cresmetas->cantidad);
            $eerf = $cresmetas->cantidad;
        }
...
//        }
    ...
    public function fc_actualizar()
    {
        ...
        $this->m_eerf2->fm_guardareerf($cfolio,$cp1,$cp2,$cp3,$cp4,$cp5,$cp6,$cp7,$cp8,$cp9,$cp10,
    ...
    public function fc_borrar()
    {
       $cfolio = $this->input->get('vfolio');
       $this->m_eerf2->fm_borrar($cfolio);
    }
    
    public function fc_cerrarpasoeerf2()
    {
       $cfolio = $this->input->get('vfolio');
       $cidpaso = $this->input->get('vidpaso');
       $cestado = $this->input->get('vestado');
       $this->m_eerf2->fm_cerrarpasoeerf2($cfolio,$cidpaso,$cestado);
       return 1;
    }
