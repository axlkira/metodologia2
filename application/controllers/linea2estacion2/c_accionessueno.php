...
/////////////// NUEVO LOGIN ///////////////////////////
class C_accionessueno extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('/linea2estacion2/m_accionessueno');
    }

     //funcion encargada de llamar a la vista de la pagina 
    public function fc_accionessueno($page = 'v_accionessueno') {
        ...
        $folioexiste = $this->m_accionessueno->fm_folioexiste($folio);//lo mandasmos a la funcion del modelo para verificar
        ...
        $datossuenointegrante = $this->m_accionessueno->fm_suenointegrante($folio,$idintegrante,$idsueno);//lo mandasmos a la funcion del modelo para verificar
        ...
        $datosintegrante = $this->m_accionessueno->fm_datosintegrante($folio,$idintegrante);//lo mandasmos a la funcion del modelo para verificar
        ...
        $suenoexiste = $this->m_accionessueno->fm_principalhogarsueno($folio);//lo mandasmos a la funcion del modelo para verificar
        ...
        $listacategorias = $this->m_accionessueno->fm_listarcategorias();
        $categorias = '';
        ...
        //traer una lista de valores
        $listasino = $this->m_accionessueno->fm_listarrepresentante($folio);
        $representante = '';
        ...
        //traer una lista de valores
        $listasino = $this->m_accionessueno->fm_suenotiempo();
        $motivo = '';
        foreach ($listasino as $estado)
        {
        ...
        //traer una lista de valores
        $listartabla = $this->m_accionessueno->fm_principalsuenoacciones($folio,$idsueno,$idintegrante);
        ...
        $datossuenoacciones .=  '<tr '.$colorfila.'>'
                    ...
                    .'<td><button type="button" id="acciones'.$valor->idintegrante.'" onclick="acciones('.$valor->idaccion.','.$valor->idsueno.','.$valor->folio.','.$valor->idintegrante.')" class="btn btn-success btn-sm">Editar accion</button></td>'
    ...
    public function fc_insertar()
    {
        ...
        $cnombresuenotiempo = $this->input->get('vnombresuenotiempo');
        $csuenotiempo = $this->input->get('vsuenotiempo');
        $this->m_accionessueno->fm_insertar($cidsueno,$cfolio,$cidintegrante,$cnombresuenotiempo,$csuenotiempo);
    ...
    public function fc_eliminaraccion()
    {
        ...
        $cfolio = $this->input->get('vfolio');
        $cidintegrante = $this->input->get('vidintegrante');
        $this->m_accionessueno->fm_eliminaraccion($cidaccion,$cidsueno,$cfolio,$cidintegrante);
        //echo json_encode(array('result' => 'guardado'));
    }
