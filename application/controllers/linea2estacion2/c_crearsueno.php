...
/////////////// NUEVO LOGIN ///////////////////////////
class C_crearsueno extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('/linea2estacion2/m_crearsueno');
    }

     //funcion encargada de llamar a la vista de la pagina 
    public function fc_crearsueno($page = 'v_crearsueno') {
        ...
        $datosintegrante = $this->m_crearsueno->fm_datosintegrante($folio,$idintegrante);//lo mandasmos a la funcion del modelo para verificar
        ...
        $folioexiste = $this->m_crearsueno->fm_folioexiste($folio);//lo mandasmos a la funcion del modelo para verificar
        ...
        $suenoexiste = $this->m_crearsueno->fm_principalhogarsueno($folio);//lo mandasmos a la funcion del modelo para verificar
        ...
        $listacategorias = $this->m_crearsueno->fm_listarcategorias();
        $categorias = '';
        ...
        //traer una lista de valores
        $listasino = $this->m_crearsueno->fm_listarrepresentante($folio);
        $representante = '';
        ...
        //traer una lista de valores
        $listasino = $this->m_crearsueno->fm_suenoaplica();
        $motivo = '';
        foreach ($listasino as $estado)
        {
        ...
        //fin traer una lista de valores
        //traer una lista de valores
        $listartabla = $this->m_crearsueno->fm_principalhogarsueno($folio);
        ...
        //validar si el sueño existe
        $suenovsaccion = $this->m_crearsueno->fm_suenovsaccion($folio);//lo mandasmos a la funcion del modelo para verificar
        
        //ACA USTEDES SI TOCAN - COLOCAN EL NOMBRE DE LAS VARIABLES idresc9mp1 - idresc9mp4a - idresc9mp4b - idresc9mp4c
        $suevsaccion= array('esuenosinaccion'=>'');
        foreach ($suenovsaccion as $fila)
        {
        //USTEDES SI TOCAN - LO VERDE LO COLCAN COMO ESTA EN LA TABLA DE LA BD
        $suevsaccion = array('esuenosinaccion'=>$fila->suenosinaccion); //se guarda el resultado del ption value en la variable datos
        }
        //fin validar si el sueño existe
        //ESTO LEVANTA LA VISTA     
        $titulo = 'v_crearsueno'; // para el titulo de la vista en la pestaña
        $this->load->view('pages/linea2estacion2/'.$page,  array('titulo' => $titulo, 'foot' => FOOTS, 'head1' => HEAD1, 'botonerag' => BOTONERAG, 'botoneraa' => BOTONERAA, 'botonerac' => BOTONERAC,'motivo' => $motivo, 'representante' => $representante,'folio' => $folio, 'doccogestor' => $doccogestor, 'foexiste' =>$foexiste, 'tdatinte' => $tdatinte, 'suexiste' => $suexiste, 'categorias' => $categorias,'datossueno'=>$datossueno,'suevsaccion'=>$suevsaccion));
    ...
    public function fc_insertar()
    {
        ...
        $this->m_crearsueno->fm_insertar($cfolio,$cidintegrante,$caplica,$cnombresueno,$ccatsueno,$crecursosinto1,$crecursosinto2,$crecursosinto3,$crecursosinto4,$crecursosinto5,$crecursosinto6,$crecursosinto7,$crecursosinto8,$crecursosinto9,$crecursosexto1,$crecursosexto2,$crecursosexto3,$crecursosexto4,$crecursosexto5);
    ...
    public function fc_eliminarsueno()
    {
        ...
        $cfolio = $this->input->get('vfolio');
        $cidintegrante = $this->input->get('vidintegrante');
        $this->m_crearsueno->fm_eliminarsueno($cid,$cfolio,$cidintegrante);
        //echo json_
        //encode(array('result' => 'guardado'));
    }
    ...
    public function fc_cerrarpaso()
    {
       ...
       $this->m_crearsueno->fm_cerrarpaso($cfolio,$cidpaso,$cestado);
    }
