...
class  C_ppdiligenciarintegrante extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->helper('url');
        $this->load->library('session');
        $this->load->model('/perfilproductivo/m_ppdiligenciarintegrante');//carga el controlador modelo
...
//
    ...
    public function fc_ppdiligenciarintegrante($page = 'v_ppdiligenciarintegrante') {
       ...
       // $this->c_capitulosintegrante->fc_capitulosintegrante();
        ...
        $datosintegrante = $this->m_ppdiligenciarintegrante->fm_datosintegrante($folio,$idintegrante);//lo mandasmos a la funcion del modelo para verificar
        ...
        $idinteexiste = $this->m_ppdiligenciarintegrante->fm_foloexiste($idintegrante);//lo mandasmos a la funcion del modelo para verificar
        ...
        $crespuestas = $this->m_ppdiligenciarintegrante->fm_buscarpregunta($folio);
        ...
        $listartabla = $this->m_ppdiligenciarintegrante->fm_listartabla($folio);
...
//        $resultado = $this->m_ppdiligenciarintegrante->fm_estadofin($folio);
...
//        
       ...
       //ESTO LEVANTA LA VISTA     
        $titulo = 'v_ppdiligenciarintegrante'; // para el titulo de la vista en la pestaña
        $this->load->view('pages/perfilproductivo/' . $page,  array('titulo' => $titulo, 'foot' => FOOTS, 'head3' => HEAD3, 'botonerag' => BOTONERAG, 'botoneraa' => BOTONERAA, 'datos'=> $datos,  'folio' => $folio, 'foexiste' =>$foexiste, 'tdatinte' => $tdatinte,'estadofin' => $estadofin,'arrayrespuestas' => $carrayrespuestas));
    ...
    public function fc_insertar()
    {
        ...
        $this->m_ppdiligenciarintegrante->fm_insertar($cfolio,$cidintegrante,$ctdocumento,$cndocumento,$cpnombre,$csnombre,$cpapellido,$csapellido,$cfnacimiento,$csexo,$cedad,$ctelefono,$ccelular,$ccorreo,$ctwitter,$cfacebook,$cinstagram,$cgoogle);
    ...
    public function fc_actualizar()
    {
        ...
        $cinstagram = $this->input->get('vinstagram');
        $cgoogle = $this->input->get('vgoogle');
        $this->m_ppdiligenciarintegrante->fm_actualizar($cfolio,$cidintegrante,$ctdocumento,$cndocumento,$cpnombre,$csnombre,$cpapellido,$csapellido,$cfnacimiento,$csexo,$cedad,$ctelefono,$ccelular,$ccorreo,$ctwitter,$cfacebook,$cinstagram,$cgoogle);
        //echo json_encode(array('result' => 'guardado'));
    ...
    public function fc_cerrarpaso()
    {
       ...
       $this->m_ppdiligenciarintegrante->fm_cerrarpaso($cfolio,$cidpaso,$cestado);
    }
    
    
    public function fc_llenarppcapitulosintegrante() {
        $cfolio = $this->input->get('vfolio');
        $cidintegrante = $this->input->get('vidintegrante');
        $this->m_ppdiligenciarintegrante->fm_llenarppcapitulosintegrante($cfolio, $cidintegrante);
        //echo json_encode(array('result' => 'guardado'));
     }
