...
/////////////// NUEVO LOGIN ///////////////////////////
class C_actualizarpp extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('perfilproductivo/m_actualizarpp');
    }

     //funcion encargada de llamar a la vista de la pagina 
    public function fc_actualizarpp($page = 'v_actualizarpp') {
        ...
        $datosintegrante = $this->m_actualizarpp->fm_datosintegrante($folio,$idintegrante);//lo mandasmos a la funcion del modelo para verificar
        ...
        $folioexiste = $this->m_actualizarpp->fm_folioexiste($folio);//lo mandasmos a la funcion del modelo para verificar
        ...
        //validar si el folio existe
        $actualizoexiste = $this->m_actualizarpp->fm_actualizoexiste($folio);//lo mandasmos a la funcion del modelo para verificar
        
        //ACA USTEDES SI TOCAN - COLOCAN EL NOMBRE DE LAS VARIABLES idresc9mp1 - idresc9mp4a - idresc9mp4b - idresc9mp4c
           $actexiste= array('efolio' => '','eactualizo' => ''); 
        
        foreach ($actualizoexiste as $fila)
        {
        //USTEDES SI TOCAN - LO VERDE LO COLCAN COMO ESTA EN LA TABLA DE LA BD
        $actexiste = array('eactualizo' => $fila->actualizo); //se guarda el resultado del ption value en la variable datos
 
        }
 
        //traer una lista de valores
        $listasino = $this->m_actualizarpp->fm_listarrepresentante($folio);
        $representante = '';
        ...
        //traer una lista de valores
        $listasino = $this->m_actualizarpp->fm_listarmotivo();
        $motivo = '';
        foreach ($listasino as $estado)
        {
        ...
        //traer una lista de valores
        $listasino = $this->m_actualizarpp->fm_listarestrato();
        $estratos = '';
        foreach ($listasino as $estado)
        {
        ...
        //traer una lista de valores
        $listasino = $this->m_actualizarpp->fm_listarrelcont();
        $relcont = '';
        foreach ($listasino as $estado)
        {
        ...
        //fin traer una lista de valores            
         ...
         //traer una lista de valores
        $listacomunas = $this->m_actualizarpp->fm_listarcomunas();
        $comunas = '';
    ...
    public function fc_traerbarrios()
    {
        ...
        $listasino = $this->m_actualizarpp->fm_listarbarrios($cidcomuna);
        $barrios = '';
        foreach ($listasino as $estado)
        {
    ...
    public function fc_actualizarh()
    {
        ...
        $this->m_actualizarpp->fm_actualizarh($cfolio,$cestrato,$ccomuna,$cbarrio,$cdireccion,$ctelefono,$ccelular,$ctelfcont,$ccelcont);
    ...
    public function fc_cerrarpaso()
    {
       ...
       $this->m_actualizarpp->fm_cerrarpaso($cfolio,$cidpaso,$cestado);
    }
    ...
    public function fc_cerrarpaso1()
    {
       ...
       $cidpaso = $this->input->get('vidpaso');
       $cestado = $this->input->get('vestado');
       $this->m_actualizarpp->fm_cerrarpaso1($cfolio,$cidpaso,$cestado);
    }
