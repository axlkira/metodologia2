...
class  C_actualizarint extends CI_Controller {
    ...
    public function fc_actualizarint($page = 'v_actualizarint') {
        ...
        {
         if($valor->estadointegrante =='1')
         {
         $estadoint = 'Activo';    
         }
         else if($valor->estadointegrante =='6')
         {
          $estadoint = 'Inactivo';   
         }
         else if($valor->estadointegrante =='5')
         {
          $estadoint = 'Inactivo por folio';   
         }
        $datos .=  '<tr>'
                    .'<td>'.$valor->folio.'</td>'
        ...
        $this->load->view('pages/actualizar/' . $page,  array('titulo' => $titulo, 'foot' => FOOTS, 'head44' => HEAD44, 'botonerag' => BOTONERAG, 'botoneraa' => BOTONERAA, 'datos'=> $datos,  'folio' => $folio,'doccogestor' => $doccogestor, 'foexiste' =>$foexiste, 'tdatinte' => $tdatinte,'estadoint' => $estadoint));             
